<x-app-layout>
    <x-slot name="title">Permintaan Pembayaran</x-slot>

    <div class="w-full px-4 md:px-8 py-8 font-sans">
        
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-[#0F172B]">Permintaan Pembayaran</h1>
                <p class="text-sm text-[#64748B] mt-1">Daftar tagihan termin (Progress Payment).</p>
            </div>
                ...
                <table class="w-full text-sm text-left">
                    <thead class="text-xs text-[#64748B] uppercase bg-[#F8FAFC] border-b border-[#E7E3DC]">
                        <tr>
                            ...
                            <th class="px-6 py-3 font-semibold">No Tagihan</th>
                            <th class="px-6 py-3 font-semibold">Proyek</th>
                            <th class="px-6 py-3 font-semibold text-center">Termin Ke</th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-[#E7E3DC] even:bg-[#F8FAFC] hover:bg-[#F1F5F9] transition-colors">
                            <td class="px-6 py-4 text-[#64748B] text-xs">{{ $pemb->tanggal_pengajuan->isoFormat('D MMM Y') }}</td>
                            <td class="px-6 py-4 text-[#0F172B] font-medium whitespace-nowrap">{{ $pemb->nomor_tagihan }}</td>
                            <td class="px-6 py-4 text-[#64748B] text-xs font-medium">{{ $pemb->proyek->nama_proyek }}</td>
                            <td class="px-6 py-4 text-center font-bold">{{ $pemb->termin_ke }} <br><span class="font-normal text-xs text-[#64748B]">({{$pemb->persentase_kemajuan}}%)</span></td>
                            <td class="px-6 py-4 text-[#0F172B] font-mono font-bold text-right">Rp {{ number_format($pemb->nilai_tagihan, 0, ',', '.') }}</td>
                            <td class="px-6 py-4 text-center">
                                @if($pemb->status == 'disetujui_ppk')
                                ...
                                <div class="flex gap-2 justify-center" x-data="{ openReviewPPK: false }">
                                    ...
                                    <div x-show="openReviewPPK" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" style="display: none;">
                                        <div @click.away="openReviewPPK = false" class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md text-left">
                                            <h3 class="text-lg font-bold text-[#0F172B] mb-4">Approval Pencairan oleh PPK</h3>
                                            
                                            <form method="POST" id="form-review-ppk-{{ $pemb->id }}">
                                                @csrf
                                                <div class="mb-4">
                                                ...
                                                <div class="flex justify-end gap-3">
                                                    ...
                                                    <button type="submit" formaction="{{ route('permintaan-pembayaran.reject-ppk', $pemb) }}" class="px-4 py-2 text-sm bg-red-50 text-red-600 border border-red-200 rounded font-semibold hover:bg-red-100">Tolak Pembayaran</button>
                        ...
                        @empty
                        <tr>
                            <td colspan="7" class="px-6 py-8 text-center text-[#64748B] italic">Belum ada tagihan pembayaran.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <x-modal name="create-pembayaran" focusable>
            <form method="POST" action="{{ route('permintaan-pembayaran.store') }}" class="p-6">
                @csrf
                <h2 class="text-lg font-bold text-[#0F172B] mb-4">Buat Tagihan Termin Baru</h2>
                ...
                <div class="mb-4 flex gap-4">
                    <div class="w-2/3">
                        <label class="block text-xs font-semibold text-[#64748B] uppercase tracking-wider mb-2">Nomor Tagihan / Invoice</label>
                        <input type="text" name="nomor_tagihan" required class="w-full border-[#E7E3DC] rounded focus:ring-[#FFA000] focus:border-[#FFA000] text-sm text-[#0F172B]" placeholder="INV/...">
                    </div>
                    <div class="w-1/3">
                        <label class="block text-xs font-semibold text-[#64748B] uppercase tracking-wider mb-2">Termin Ke</label>
                        <input type="number" name="termin_ke" required min="1" class="w-full border-[#E7E3DC] rounded focus:ring-[#FFA000] focus:border-[#FFA000] text-sm text-[#0F172B]">
                    </div>
                </div>

                <div class="mb-4 flex gap-4">
                    <div class="w-1/2">
                        <label class="block text-xs font-semibold text-[#64748B] uppercase tracking-wider mb-2">Progress Lapangan (%)</label>
                        <input type="number" name="persentase_kemajuan" required min="0" max="100" step="0.01" class="w-full border-[#E7E3DC] rounded focus:ring-[#FFA000] focus:border-[#FFA000] text-sm text-[#0F172B]" placeholder="0 - 100">
                    </div>
                    <div class="w-1/2">
                        <label class="block text-xs font-semibold text-[#64748B] uppercase tracking-wider mb-2">Nilai Tagihan (Rp)</label>
                        <input type="number" name="nilai_tagihan" required min="0" class="w-full border-[#E7E3DC] rounded focus:ring-[#FFA000] focus:border-[#FFA000] text-sm text-[#0F172B] text-right" placeholder="Misal: 100000000">
                    </div>
                </div>
